        ...
        first_reconfigure_call_(true)
{
  ...
  odom_pub = nh_.advertise<nav_msgs::Odometry>("pose", 1, true);
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  if(!pf_init_)
  {
    ...
    pf_odom_pose_ = pose;
    latest_tf_ =tf::Transform(tf::createQuaternionFromRPY(0, 0, 0), tf::Vector3(0, 0, 0.0));
  ...
  if(resampled || force_publication)
  {
      ...
      if(max_weight > 0.0)
      {
        ...
        */
        ...
        try
        {
          tf::Transform tmp_tf(tf::createQuaternionFromYaw(0),//hyps[max_weight_hyp].pf_pose_mean.v[2]),
                             tf::Vector3(0,//hyps[max_weight_hyp].pf_pose_mean.v[0],
                                         0,//hyps[max_weight_hyp].pf_pose_mean.v[1],
                                         0.0));
        ...
        latest_tf_ = tf::Transform(tf::createQuaternionFromRPY(0, 0, 0), tf::Vector3(0, 0, 0.0));//tf::Transform(tf::Quaternion(odom_to_map.getRotation()),
  ...
  try
  {
      tfodom_.lookupTransform("/odom", "/base_footprint", ros::Time(0), t);
  }
  ...
  odom_to_laser.setOrigin( tf::Vector3(t.getOrigin().x(), t.getOrigin().y(), 0.0) );
  odom_to_laser.setRotation(t.getRotation());  
  tf::Transform map_to_laser= latest_tf_.inverse()*odom_to_laser;                                                                                                                                       
  odom.pose.pose.position.x = map_to_laser.getOrigin().x();
  odom.pose.pose.position.y = map_to_laser.getOrigin().y();
  odom.pose.pose.position.z = 0;
  odom.pose.pose.orientation.w = tf::getYaw(map_to_laser.getRotation());
