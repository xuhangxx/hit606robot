...
void SlamGMapping::startLiveSlam()
{
  ...
  lun_sub = node_.subscribe("lun",1,&SlamGMapping::lun_callback, this);
  odom_pub = node_.advertise<nav_msgs::Odometry>("pose", 1, true);

  transform_thread_ = new boost::thread(boost::bind(&SlamGMapping::publishLoop, this, transform_publish_period_));
}

void SlamGMapping::lun_callback(const control_node::lun& lun)
{
   flag=lun.n;
...
SlamGMapping::laserCallback(const sensor_msgs::LaserScan::ConstPtr& scan)
{
  ...
  tf::Transform map_to_laser=odom_to_laser;
  odom.pose.pose.position.x = odom_to_laser.getOrigin().x();
  odom.pose.pose.position.y = odom_to_laser.getOrigin().y();
  odom.pose.pose.position.z = 0;
  odom.pose.pose.orientation.w = tf::getYaw(odom_to_laser.getRotation());
