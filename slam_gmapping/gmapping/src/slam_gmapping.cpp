...
void SlamGMapping::startLiveSlam()
{
  ...
  soc_sub = node_.subscribe("/soc",1,&SlamGMapping::soc_callback, this); 
  map2_pub = node_.advertise<gmapping::map2>("map2", 1, true);

  //transform_thread_ = new boost::thread(boost::bind(&SlamGMapping::publishLoop, this, transform_publish_period_));
}

void SlamGMapping::soc_callback(const indoor_navigation::soc& soc)
{
   if(soc.code == 2 && soc.action == 1)
   { 
...
SlamGMapping::getOdomPose(GMapping::OrientedPoint& gmap_pose, const ros::Time& t)
{
  ...
  //std::cout<<"odompose"<<odom_pose.getOrigin().x()<<std::endl;
...
SlamGMapping::initMapper(const sensor_msgs::LaserScan& scan)
{
  ...
  base_to_laser=tf::Transform(tf::createQuaternionFromRPY(0, 0, tf::getYaw(laser_pose.getRotation())), tf::Vector3(laser_pose.getOrigin().x(), laser_pose.getOrigin().y(), laser_pose.getOrigin().z()));
...
SlamGMapping::addScan(const sensor_msgs::LaserScan& scan, GMapping::OrientedPoint& gmap_pose)
{
  ...
  //std::cout<<"time    4555555555555555555                "<<ros::Time::now()<<std::endl;
...
SlamGMapping::laserCallback(const sensor_msgs::LaserScan::ConstPtr& scan)
{
  ...
  //std::cout<<"time3:                                    "<<ros::Time::now()<<std::endl;
  if(socflag)
  {
    ...
    //  std::cout<<"time4:                                    "<<ros::Time::now()<<std::endl;

    if(addScan(*scan, odom_pose))
    {
      
      //std::cout<<"time 4444444444444444444444444                   "<<ros::Time::now()<<std::endl;
      ...
      //tf::Transform base_to_map = base_to_laser*tf::Transform(tf::createQuaternionFromRPY(0, 0, mpose.theta), tf::Vector3(mpose.x, mpose.y, 0.0)).inverse();
      
      tf::Transform odom_to_base = tf::Transform(tf::createQuaternionFromRPY(0, 0, odom_pose.theta), tf::Vector3(odom_pose.x, odom_pose.y, 0.0))*base_to_laser.inverse();
      tf::Transform base_to_map =odom_to_base.inverse();
      tf::Transform map_to_base = odom_to_base;
      ...
      //std::cout<<"time5:                                    "<<ros::Time::now()<<std::endl;
      ...
      //std::cout<<"x2221"<<mpose.x<<std::endl;
     // std::cout<<"x2221"<<mpose.y<<std::endl;
      //std::cout<<"x22221"<<mpose.theta<<std::endl;
      //std::cout<<"x22221"<<odom_pose.x<<std::endl;
      //std::cout<<"x22221"<<odom_pose.y<<std::endl;
      //std::cout<<"x22221"<<odom_pose.theta<<std::endl;
    } 
    else
...
SlamGMapping::updateMap(const sensor_msgs::LaserScan& scan)
{
  ...
  if(map_.map.info.width != (unsigned int) smap.getMapSizeX() || map_.map.info.height != (unsigned int) smap.getMapSizeY()) {
   ...
   // map2.data.resize(map_.map.info.width * map_.map.info.height/4);
    //map_.map.data.resize(50*50);

    ROS_DEBUG("map origin: (%f, %f)", map_.map.info.origin.position.x, map_.map.info.origin.position.y);
  }
  //dex=int(odom.pose.pose.position.x-map_.map.info.origin.position.x)*50;
  //dey=int(odom.pose.pose.position.y-map_.map.info.origin.position.y)*50;
 // std::cout<<"ssssssssssssssssssssssssssss"<<dex<<"                "<<dey<<std::endl;
 //i=0;
  for(int x=0; x < smap.getMapSizeX(); x++)
  {
    for(int y=0; y < smap.getMapSizeY(); y++)
    {
      ...
      //if(x>dex && x<(dex+1) && y>dey && y<(dey+1))
      //{
        ...
        if(occ < 0)
        {
          ...
          //if(y%4==0 && x%4==0)
          //{
         // map2.data[MAP_IDX(map_.map.info.width, x, y)/4]+= 0;
         // }
        }
          //map_.map.data[MAP_IDX(50, x-dex, y-dey)] = -1;
        else if(occ > occ_thresh_)
        {
          //map_.map.data[MAP_IDX(map_.map.info.width, x, y)] = (int)round(occ*100.0);
          map_.map.data[MAP_IDX(map_.map.info.width, x, y)] = 100;
          //map2.data[MAP_IDX(map_.map.info.width, x, y)/4]+= pow(4, i)*1;
  
          //map_.map.data[MAP_IDX(50, x-dex, y-dey)] = 100;
        }
        else
        {
          map_.map.data[MAP_IDX(map_.map.info.width, x, y)] = 0;
          //map2.data[MAP_IDX(map_.map.info.width, x, y)/4]+= pow(4, i)*2;
          //map_.map.data[MAP_IDX(50, x-dex, y-dey)] = 0;
        }
        //i--;
        //if(i==0)
        //{
