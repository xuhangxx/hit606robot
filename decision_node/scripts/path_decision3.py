...
#计算目标偏移角度
def calculate_delta (cur_x, cur_y, tar_x, tar_y, current_yaw_angle):
    heading_x = math.sin(current_yaw_angle * math.pi / 180) + cur_x
    heading_y = math.cos(current_yaw_angle * math.pi / 180) + cur_y
    
    length_cur_tar = math.sqrt((cur_x - tar_x)*(cur_x - tar_x)+ (cur_y - tar_y)*(cur_y - tar_y))
    delta_abs = math.acos( ( (heading_x - cur_x)*(tar_x - cur_x) + (heading_y - cur_y)*(tar_y - cur_y) )/(1*length_cur_tar) )
    outer_prod = (heading_x - cur_x)*(tar_y - cur_y) - (heading_y - cur_y)*(tar_x - cur_x)
    ...
    return dir*delta_abs*180/math.pi
...
def callback1(gps_data):
           ...
           latitude=gps_data.latitude-x0
           longitude=gps_data.longitude-y0
           global xr,yr
...
def path_decison_test():
    ...
    #filename='/home/robot/catkin_805/src/decision_node/data/1123gps_lat&lon'
    filename='/home/kaanh2019/catkin_ws805/src/decision_node/data/20200526gps_data'
    #filename='/home/zsj/ROS/catkin_apollo805/src/decision_node/data/1123gps_lat&lon'
	...
	value=[float(s) for s in line.split( )]
        Easting,Northing=LL2UTM(a,c,value[0]-x0,value[1]-y0,0,0)
    ...
    lastIndex = len(cx) - 1
    global xr,yr,yawr,distance,yaw_angle
    xr,yr=LL2UTM(a,c,0,0,0,0)
    yawr=deltayaw
    #yawr = deltayaw  #初始化=0.0  
    distance= deltadistance
    yaw_angle = deltayaw_angle
    ...
    while not rospy.is_shutdown():   #lastIndex > target_ind and not
        ...
        #新增 目标相对当前位置的角度 delta_test 注意：角度值正 目标在右
       
        delta_test = calculate_delta (xr, yr, cx[target_ind], cy[target_ind], state.yaw)
        rospy.loginfo("delta_test : %f" % (delta_test))
        if distance==0 :
            if delta_test >=15.0 :      #目标点在当前点的右侧 执行右转程序
                cmd_vel=Twist()
                cmd_vel.linear.x=v1
                cmd_vel.linear.y=0
            ...
            elif delta_test<=-15.0 :     
                cmd_vel=Twist()
                cmd_vel.linear.x=v1
                cmd_vel.linear.y=0
                cmd_vel.angular.z=w0
                rospy.loginfo("Turn left <----------- vx: %f, wz %f" % (v1, w0))
                cmdvel_pub.publish(cmd_vel)
            else :
                cmd_vel=Twist()
                cmd_vel.linear.x=v0
            ...
            cmd_vel.linear.y=0
            cmd_vel.angular.z=0
            rospy.loginfo("Stop！ ××××××××××××××××××  vx: %f, wz %f" % (0, 0)) 
            cmdvel_pub.publish(cmd_vel)
