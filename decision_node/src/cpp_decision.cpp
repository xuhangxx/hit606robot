...
//全局变量
vector<double> gps_x_longitude; // 存储读取的GPS轨迹
vector<double> gps_y_latitude;

vector<double> utm_x; // 存储UTM轨迹
vector<double> utm_y;


double origin_x; //GPS原点坐标
double origin_y;

double ll2utm_x = 111000.0; // GPS转UTM常量
double ll2utm_y = 111000.0;

double current_x_longitude; // 当前位置点经纬度值
double current_y_latitude;
double current_utm_x; // 当前位置点UTM坐标
...
// 工具函数
double LL2UTM_respective(double now_, double origin_)
{
    return ll2utm_x*(now_ - origin_); 
}
...
//两个callback将会顺序执行 ，先GPS后IMU
    ...
    {
        current_utm_x = LL2UTM_respective(msg->longitude, origin_x);
        current_utm_y = LL2UTM_respective(msg->latitude, origin_y);
    }
...
// 找下一个点
    void find_target()
    {
        ...
        */
        SubsGPSIMUPubControlCmdvel::get_nearest_point( current_utm_x, current_utm_y ); // 在函数中更新目标utm坐标值
...
// 寻找与传入点最近的 轨迹上的点, 并向后推移一定距离作为目标点
    void get_nearest_point( double heading_next_utm_x, double heading_next_utm_y )
    {
        double dis_min = 10;
        int index_min;
        int index_final;
...
int main( int argc, char** argv)
{
    ...
    while (!in.eof())
    {
        ...
        in >> gpsy;
        in >> gpsx;
        gps_x_longitude.push_back(gpsx);
        gps_y_latitude.push_back(gpsy);
    }
    //建立原点经纬度
    origin_x = gps_x_longitude[0];
    origin_y = gps_y_latitude[0];
    //去原点utm坐标
    for (auto x_:gps_x_longitude)
    {
        utm_x.push_back(LL2UTM_respective(x_,origin_x));
    }
    for (auto y_:gps_y_latitude)
    {
        utm_y.push_back(LL2UTM_respective(y_,origin_y));
    }
